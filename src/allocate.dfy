...
method applyOp(bal: seq<int>, from: int, to: int, amount: int) returns (res: seq<int>)
  ...
  requires (to < |bal|)
  ensures (|res| == |bal|)
  ensures (sumTo(res, |res|) == sumTo(bal, |bal|))
{
  var result := bal;
  SumToUpdate(result, from, |result|, - amount);
  result := result[from := (result[from] - amount)];
  SumToUpdate(result, to, |result|, amount);
  result := result[to := (result[to] + amount)];
  return result;
}

method replay(froms: seq<int>, tos: seq<int>, amounts: seq<int>, n: int) returns (res: seq<int>)
  requires (n >= 1)
  requires (|froms| == |tos|)
  requires (|tos| == |amounts|)
  requires forall k: int :: ((0 <= k) ==> (k < |froms|) ==> ((0 <= froms[k]) && (froms[k] < n)))
  requires forall k: int :: ((0 <= k) ==> (k < |tos|) ==> ((0 <= tos[k]) && (tos[k] < n)))
  ensures (|res| == n)
  ensures (sumTo(res, n) == 0)
{
  var bal: seq<int> := [];
  var p := 0;
  while (p < n)
